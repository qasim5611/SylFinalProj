...
import {sendFORSAVEStdInformation} from './../Redux/Action/action';
  ...
  handle=(event)=>{
    ...
    this.setState({[name]:value});
      ...
      checkReqFields=()=>{
        ...
        if(dob.trim()==""){
          document.getElementById("show").innerHTML="* DOB is not match.";
          returnValue=false;
        }
        if(subject.trim()==""){
          document.getElementById("show").innerHTML="* Subject is not match.";
          returnValue=false;
        }
        if(address.trim()==""){
          document.getElementById("show").innerHTML="* Address is not match.";
          returnValue=false;
        }								
        return returnValue;
      } 
      ...
      this.props.sendFORSAVEStdInformation(obj);   //action waly AddRecordToStore function ko call kia,
...
<html style={{height:"100%"}}>
  ...
  <Row>
    ...
    <center><p style={{paddingTop:"30px", paddingLeft:"50px", color:"white", fontFamily:"cursive", fontSize:"40px"}} >Student Information</p></center>
     </Col>
...
<List component="nav"  >
   ...
   <Link to="/Edit">    <ListItemText primary="Edit" /> </Link>
    ...
    <Row style={{height:'px', width:'px'}}> 
                            <Col xs={12} sm={6} lg={6} md={6} style={{ height:'70px' ,backgroundColor:''}}><FormControl margin="normal" required="required" fullWidth style={{marginLeft:"50px"}}>
            <InputLabel htmlFor="sub">Subject Want To Study</InputLabel>
            <Input id="sub" name="sub" onChange={this.handle} required="required" autoComplete="email" autoFocus required style={{width:"300px"}} />
        </FormControl></Col>
    ...
    <Row>
      ...
      <Button
         ...
         onClick={this.sendFORSAVEStdInformation}
...
export default connect(null, {sendFORSAVEStdInformation })  (AddTodo)
